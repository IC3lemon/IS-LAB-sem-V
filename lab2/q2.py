from Crypto.Util.number import *
from Crypto.Util.Padding import *
from Crypto.Cipher import AES


S_BOX = [
    ,
]

INV_S_BOX = [
    ,
]

N_ROUNDS = {
    128: 10,
    192: 12,
    256: 14
}

RCON = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36]

def xor_bytes(a: bytes, b: bytes):
    return bytes([x ^ y for x, y in zip(a, b)])

def sub_word(word: bytes):
    return bytes([S_BOX[b] for b in word])

def rot_word(word: bytes):
    return word[1:] + word[0:1]

def key_expansion(key: bytes):
    try:
        rounds = N_ROUNDS[len(key) * 8]
    except KeyError:
        raise ValueError("Invalid key length brah. Must be 16, 24, or 32 bytes.")

    Nk = len(key) // 4
    W = [key[i:i + 4] for i in range(0, len(key), 4)]
    
    for i in range(Nk, 4 * (rounds + 1)):
        temp = W[i-1]
        
        if i % Nk == 0:
            temp = sub_word(rot_word(temp))
            temp = xor_bytes(temp, bytes([RCON[i // Nk - 1], 0, 0, 0]))
        elif Nk > 6 and i % Nk == 4:
            temp = sub_word(temp)
        
        word = xor_bytes(temp, W[i - Nk])
        W.append(word)

    round_keys = [b''.join(W[i:i + 4]) for i in range(0, 4 * (rounds + 1), 4)]
    print(round_keys)
    return round_keys

def sub_bytes(state):
    new_state = []
    for row in state:
        new_row = [S_BOX[b] for b in row]
        new_state.append(new_row)
    return new_state

def inv_sub_bytes(state):
    new_state = []
    for row in state:
        new_row = [INV_S_BOX[b] for b in row]
        new_state.append(new_row)
    return new_state

def shift_rows(state):
    return [
        state[0],
        state[1][1:] + state[1][:1],
        state[2][2:] + state[2][:2],
        state[3][3:] + state[3][:3]
    ]

def inv_shift_rows(state):
    return [
        state[0],
        state[1][3:] + state[1][:3],
        state[2][2:] + state[2][:2],
        state[3][1:] + state[3][:1]
    ]
    

def g_mul(a, b): # a * b (GF(2**8)) the weird xor one though
    p = 0
    for i in range(8):
        if b & 1:
            p ^= a
        
        if a & 0x80:
            a = (a << 1) ^ 0x11b
        else:
            a <<= 1
        b >>= 1
    return p

def mix_cols(state):
    mix_matrix = [
        [0x02, 0x03, 0x01, 0x01],
        [0x01, 0x02, 0x03, 0x01],
        [0x01, 0x01, 0x02, 0x03],
        [0x03, 0x01, 0x01, 0x02]
    ]
    
    new_state = [[0, 0, 0, 0] for _ in range(4)]
    for c in range(4):
        for r in range(4):
            val = 0
            for i in range(4):
                val ^= g_mul(mix_matrix[r][i], state[i][c])
            new_state[r][c] = val
            
    return new_state

def inv_mix_cols(state):
    inv_mix_matrix = [
        [0x0e, 0x0b, 0x0d, 0x09],
        [0x09, 0x0e, 0x0b, 0x0d],
        [0x0d, 0x09, 0x0e, 0x0b],
        [0x0b, 0x0d, 0x09, 0x0e]
    ]

    new_state = [[0, 0, 0, 0] for _ in range(4)]
    for c in range(4):
        for r in range(4):
            val = 0
            for i in range(4):
                val ^= g_mul(inv_mix_matrix[r][i], state[i][c])
            new_state[r][c] = val
            
    return new_state

def add_round_key(state, round_key):
    key_matrix = [list(bytearray(round_key[i:i+4])) for i in range(0, len(round_key), 4)]
    new_state = [[0, 0, 0, 0] for _ in range(4)]
    for r in range(4):
        for c in range(4):
            new_state[r][c] = state[r][c] ^ key_matrix[c][r]
    return new_state

def transpose(state):
    return [list(x) for x in zip(*state)]

def encrypt_block(block: bytes, round_keys: list):
    rounds = N_ROUNDS[len(round_keys[0]) * 8]
    
    state = add_round_key(transpose([list(bytearray(block[i:i+4])) for i in range(0, len(block), 4)]), round_keys[0])
    
    for i in range(1, rounds):
        state = add_round_key(mix_cols(shift_rows(sub_bytes(state))), round_keys[i])

    state = add_round_key(shift_rows(sub_bytes(state)), round_keys[rounds])

    enc = b''
    for c in range(4):
        for r in range(4):
            enc += bytes([state[r][c]])

    return enc

def decrypt_block(block: bytes, round_keys: list):
    rounds = N_ROUNDS[len(round_keys[0]) * 8]

    state = transpose([list(bytearray(block[i:i+4])) for i in range(0, len(block), 4)])
    state = inv_sub_bytes(inv_shift_rows(add_round_key(state, round_keys[rounds])))

    for i in range(rounds - 1, 0, -1):
        state = inv_sub_bytes(inv_shift_rows(inv_mix_cols(add_round_key(state, round_keys[i]))))

    state = add_round_key(state, round_keys[0])

    dec = b''
    for c in range(4):
        for r in range(4):
            dec += bytes([state[r][c]])
            
    return dec

def encrypt(data: bytes, key: bytes):
    round_keys = key_expansion(key)
    padded_data = pad(data, 16)
    encrypted_data = b''
    for i in range(0, len(padded_data), 16):
        block = padded_data[i:i+16]
        encrypted_data += encrypt_block(block, round_keys)
    return encrypted_data

def decrypt(data: bytes, key: bytes):
    round_keys = key_expansion(key)
    decrypted_data = b''
    for i in range(0, len(data), 16):
        block = data[i:i+16]
        decrypted_data += decrypt_block(block, round_keys)
    return unpad(decrypted_data, 16)

if __name__ == "__main__":
    msg = pad(b'pakoda', 16)
    ct0 = encrypt(msg, b'a'*16)[:len(msg)] 
    print(ct0)

    c = AES.new(b'a'*16, AES.MODE_ECB)
    ct1 = c.encrypt(msg)
    print(ct1)

    assert ct0 == ct1 
    print('aight lesgo')
